...
export const LessonView: React.FC<LessonViewProps> = ({
  ...
  // States to control interactive progression
  const [speakCount, setSpeakCount] = useState<number>(0);
  const [userTranslation, setUserTranslation] = useState<string>('');
  const [translationChecked, setTranslationChecked] = useState<boolean>(false);
  const [isTranslationCorrect, setIsTranslationCorrect] = useState<boolean>(false);
  const [showHint, setShowHint] = useState<boolean>(false);

  // Normalizes strings for matching (strips punctuation, lowers case, trims extra spacing)
  const checkTranslation = () => {
    const cleanUser = userTranslation.trim().toLowerCase().replace(/[.,\/#!$%\^&\*;:{}=\-_`~()?]/g, "").replace(/\s+/g, " ");
    
    // Find matching practice targets
    const correctTarget = lesson.translationPractice[0]?.answer.trim().toLowerCase().replace(/[.,\/#!$%\^&\*;:{}=\-_`~()?]/g, "").replace(/\s+/g, " ") || "";

    const isMatch = cleanUser === correctTarget;
    setIsTranslationCorrect(isMatch);
  ...
  const isStep1Done = true; // Read is always done
  const isStep2Done = speakCount >= 5;
  const isStep3Done = translationChecked && isTranslationCorrect;
    ...
    <div className="relative z-10 max-w-4xl mx-auto px-4 py-8">
      ...
      >
        <div className="flex justify-between items-center gap-4">
          <div>
            <span className="text-xs font-black uppercase text-indigo-400 tracking-widest">Lesson {lesson.id}</span>
      ...
      <div className="space-y-8">
            ...
            {lesson.sentences.map((sent, sIdx) => (
              <div key={sIdx} className="glass-panel rounded-2xl p-5 border border-white/5 bg-slate-950/20 space-y-4">
                <div className="flex items-start gap-4">
                  <div className="w-8 h-8 rounded-full bg-amber-400/10 text-amber-400 flex items-center justify-center flex-shrink-0 text-sm font-bold">A</div>
                  <div>
        ...
        >
          ...
          {/* Interactive 5-Time Tap tracker */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-2xl bg-slate-950/40 border border-white/5">
            <div>
              ...
              <p className="text-[11px] font-content text-slate-400 mt-1">Read out loud, and tap the count bubble to complete.</p>
            </div>
        ...
        {/* STEP 4: Translate Practice (Tamil to English) */}
        <motion.section 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <div className="glass-panel rounded-2xl p-5 border border-white/5 bg-slate-900/30 mb-5 flex justify-between items-center">
            <div>
              <span className="text-[10px] text-slate-500 uppercase tracking-widest block mb-1">Tamil Input Sentence</span>
              <span className="text-lg font-extrabold text-amber-400 font-content">{lesson.translationPractice[0]?.question}</span>
            </div>
          </div>

          <div className="space-y-4">
            <div>
              <label htmlFor="user-trans-input" className="text-xs text-slate-400 uppercase tracking-wider block mb-2 font-semibold">Write the English Sentence below:</label>
              <input
                id="user-trans-input"
                type="text"
                ...
                className="w-full bg-slate-950/60 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-slate-600 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all font-semibold font-interface"
              />
            </div>

            {/* Warning or Correct Notification */}
                ...
                >
                  {isTranslationCorrect ? (
                    <>
                      ...
                      <span>Very Good Student! Intha translation exact ah right! 🎉</span>
                    </>
                  ) : (
                    <>
                      <AlertCircle className="w-5 h-5 flex-shrink-0" />
                      <span>Oops! Keep trying. spelling and punctuation check pannunga.</span>
                    </>
                  )}
                </motion.div>
              )}
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              >
                {showHint ? "Hide Answer Hint 💡" : "Show Answer Hint 💡"}
              </button>

              <button
                type="button"
                disabled={userTranslation.trim() === '' || isStep3Done}
              ...
              >
                {isStep3Done ? "Checked!" : "Check Answer"}
              </button>
            </div>
            ...
            <AnimatePresence>
              {showHint && (
                ...
                >
                  <div className="p-4 rounded-xl bg-slate-900 border border-white/5 text-xs text-slate-400 mt-2 font-content">
                    <span className="font-bold text-indigo-300 uppercase block mb-1">Target Translation Hint:</span>
                    <span>It starts with &quot;{lesson.translationPractice[0]?.answer.split(' ')[0]}&quot; and ends with &quot;{lesson.translationPractice[0]?.answer.split(' ').pop()}&quot;. Target: <strong className="text-white">&quot;{lesson.translationPractice[0]?.answer}&quot;</strong></span>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
